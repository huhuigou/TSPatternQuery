...
The rest of this document will be laid out as follows. The first section will go into further detail explaining the underlying algorithm. The second section will present present the different functions made available through the TSPatternQuery. This section will also briefly mention the CreateCustomTimeSeries function that has been provided to help the user in creating pattern templates. The last section will discuss best practices and provide examples.
...
Users are strongly recommended to test their *ruleset* functions before using them with a large data set. See the Best Practices and Examples section below for more info on how to test these functions. 
...
##Testing *pattern.template*s, *distinctive.feature*s and *ruleset*s
TODO: Demonstrate creating several time series that should and should not satisfy a function and testing on that. Discuss RUnit as a more formal a rigid way to test functions, especially if they have multiple and/or complex functions and would like automated scripts to verify they still work every time they change something.
...
Below are several examples of simple functions that may be passed to the *distinctive.feature* parameter in order to reduce the run time of the algorithm. The first function will include a worked example.

###A Worked Example: Exclusion Based on Variance

If the variance of the pattern significantly differs from that of the surrounding time series, then excluding low (and/or high) variance segments can be a very effective distinctive feature.
...
```{r, echo=TRUE, fig.align='right', fig.show='hold', out.width='50%'}
...
```{r, echo=FALSE, fig.align='right', fig.show='hold', out.width='50%'}
