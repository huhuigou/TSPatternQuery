---
title: "A Pattern Querying Tool for Time Series Using PIPs and Hybrid Template-Ruleset Matching"
...
#Introduction
TSPatternQuery is a Time Series Pattern Querying Tool meant for quick exploration of time series patterns. This package can be viewed as an implementation of the "real time hybrid pattern matching scheme" presented in [@Zhang:2010:RTH:1862242.1862263]. The following example illustrates how TSPatternQuery is used:

```{r, echo=FALSE, fig.show='hold', out.width='50%'}

path <- system.file("extdata", "introduction_ts.csv", package = "TSPatternQuery")
example.timeseries<- xts(read.csv(path)[[2]],order.by = as.POSIXct(read.csv(path)[[1]]))
plot(
  example.timeseries,
  main="example.timeseries",
  xlab = "Seconds",
  ylab = "Value"
)

example.pattern <- CreateCustomTimeSeries(
  values = c(0, -1, 2, -4, 2, -1, 0),
  intervals = c(5,5,5,5,5,5)
)
plot(
  example.pattern,
  main="example.pattern",
  xlab = "Seconds",
  ylab = "Value"
)
```

```{r, echo=TRUE, fig.show='hold', fig.align='center' ,out.width='50%'}
...
TSPatternQuery uses a sliding window algorithm. Important points are identified within the window at each offset. They are then compared to a template of the desired pattern using Spearman's rho as a similarity measure. Additional rules provided to the function may be used to increase accuracy, or to short circuit the algorithm in order to increase speed.

The rest of this document will be laid out as follows. The first section will go into further detail explaining the underlying algorithm. The second section will present the different functions made available through the TSPatternQuery package. This section will also briefly mention the CreateCustomTimeSeries function that has been provided to help the user in creating pattern templates. The last section will discuss best practices and provide a worked example.
...
*timeseries* is the data being queried (i.e. the time series that may or may not contain the pattern being searched for). It should be an xts time series object. *timeseries* can be of any length, and may be regular or irregular (i.e. time intervals between points do not have to be of the same length).

###*pattern.template*

The *pattern.template* is an xts time series that serves as a representation of the pattern being searched for. It should be the same length (in seconds) as the pattern is estimated to be, and consist of the minimum number of points necessary to create the pattern's shape. The first and last points in pattern.template (the endpoints) must be estimations of the time series BEFORE and AFTER the pattern has occurred, otherwise it will likely not match. For example, if the time series is normally constrained between 1 and -1, but the pattern contains points outside of this range, then a good value for the *pattern.template* endpoints would likely be 0.5.

The TSPatternQuery algorithm does not read times as absolute values. It only considers the time intervals (in seconds) between points. Thus, using a *pattern.template* with times/dates that are far removed from the time series data is perfectly acceptable. In other words, a *pattern.template* with time stamps from 2016 will still match data from 1990, so long as the shape is sufficiently similar. 
...
distinctive.feature <- function(window){
  ...
  #return TRUE if the window matches the distinctive feature
  #and FALSE otherwise
  
}
```

This function is run PRIOR to PIP identification and template based matching. If the current window does not match the *distinctive.feature*, then the algorithm will move on to the next window without identifying PIPs. Proper use of the *distinctive.feature* parameter can significantly decrease the time it takes for the algorithm to run, since it prevents the slow GetPIPs function from being called.
...
ruleset <- function(window, pips){
  ...
  #return TRUE if the window and/or pips match the ruleset
  #and FALSE otherwise
...
##GetPIPs
Perceptually important points (PIPs) are points that have been identified as perceptually important for the formation of patterns within a time series. GetPIPs is used by the Query function to identify PIPs within the sliding window at each offset. The function has been made available for users to call in order to test their custom pattern templates. 
...
```
###*timeseries*
The time series that will be check against the pattern to see if they match.
...
3. Are there any situations in which it might return NA? This will be counted as FALSE by the algorithm. Is this desirable?
4. Are there any situations in which it might throw an Error? This will be counted as FALSE by the algorithm. Is this desirable?

More rigorous testing may be performed using RUnit or testthat packages. These facilitate the creation of automated scripts containing the same sort of tests that one might type into the console, as described above. The advantage of this sort of testing is that it is automated, and may thus be run each time something is added or changed. This reduces the likelihood of things breaking unexpectedly. 
...
Below are several functions that may be passed to the *distinctive.feature* parameter. Remember that the aim of *distinctive.feature* is to reduce running time by finding false matches early in the algorithms execution (i.e. before PIP identification). The first function will include a worked example.
...
Note that the previously 1st Matched Window from before no longer matches since it does not satisfy the distinctive feature of having a variance greater than 3.5. This may or may not be desirable depending on the circumstances.
...
Excluding windows that do or don't contain points in some range can be a simple yet effective distinctive feature if the pattern is known only occur in or outside of said range.
...
Then call them all from within a single "aggregate ruleset" function. The aggregate function is passed to Query as normal.
```{r, fig.show='hold', eval=FALSE}
